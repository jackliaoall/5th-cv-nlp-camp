...
"""
--batch_size 16 --dataset KITTI --data_path ./datasets/KITTI --gpu_num 0,1,2,3

###配置环境
1.安装一些工具包：
打开anaconda安装路径中的自己当前使用的环境，然后进去lib文件夹中
新建一个fcntl.py 放在lib下

2.手动下载curses,选择对应版本然后再PIP装
https://www.lfd.uci.edu/~gohlke/pythonlibs/#curses

3.在blessings中注释掉 #from termios import TIOCGWINSZ
...
2.准备实际的训练数据图像：
还是在KITTI文件夹中找到kitti_archives_to_download文件，下载里面的数据即可,解压到KITTI文件夹中即可

3.更改训练和验证TXT文件
根据自己下载的数据和标签选择好对应的路径即可
"""
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    save_path = save_path_formatter(args, parser)#.replace(':','-')
    #print(save_path)
