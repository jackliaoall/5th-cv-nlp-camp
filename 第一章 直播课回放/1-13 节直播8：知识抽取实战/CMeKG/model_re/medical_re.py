...
class config:
    ...
    PATH_SCHEMA = "D:\\CMeKG\\predicate.json"
    PATH_TRAIN = 'D:\\CMeKG\\train_example.json'
    PATH_BERT = "D:\\CMeKG\\model\\medical_re"
    PATH_MODEL = "D:\\CMeKG\\model\\medical_re\\model_re.pkl"
    PATH_SAVE = 'D:\\CMeKG\\model\\save'
    tokenizer = BertTokenizer.from_pretrained("D:\\CMeKG\\model\\medical_re\\vocab.txt")
...
def load_schema(path):#列出来的所有关系及其对应ID
...
def extract_spoes(text, model4s, model4po):
    ...
    with torch.no_grad():
        ...
        print("decode：", tokenizer.decode(token_ids[0]))
        ...
        for s in subjects:
            ...
            object_labels_pred[0, len(text) + 2:, :, :] = 0#无用位置置0
            start = np.where(object_labels_pred[0, :, :, 0] > 0.4)#得到所有可能起始位置和其对应预测类别
            end = np.where(object_labels_pred[0, :, :, 1] > 0.4)#

            for _start, predicate1 in zip(*start):#起始位置及其关系类别
                for _end, predicate2 in zip(*end):#结束位置及其关系类别
                    if _start <= _end and predicate1 == predicate2:
                        spoes.append((s, predicate1, (_start, _end)))#一会要遍历得到结果
                        break
